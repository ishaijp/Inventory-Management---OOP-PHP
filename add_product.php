			 ...
			 <p class="msg"> 
				<?php
					if($_SERVER['REQUEST_METHOD'] == 'POST'){
						...
						if(empty($name) or empty($pquantity) or empty($pprice) or empty($puprice)){
							echo "<span style='color:#e53d37'>Error...Field Must Not Be Empty</span>";
						} else {
							$addpro = $user->updatePro($id, $name, $pquantity, $pprice, $puprice);
						    if($addpro){
...
<div class="random_table"> 
			    <form action="" method="post">
					<table>
					   ...
					   <tr>
							<td>Quantity</td>
							<td><input type="text" name="proQuantity" value="<?php echo $row['proQuantity']; ?>" /></td>
						</tr>
						<tr>
							<td>Price</td>
							<td><input type="text" name="proPrice" value="<?php echo $row['proPrice']; ?>" /></td>
						</tr>
						<tr>
							<td>Unit Price</td>
							<td><input type="text" name="proUPrice" value="<?php echo $row['proUPrice']; ?>" /></td>
						</tr>						
						<tr>
							<td colspan="2"> 
