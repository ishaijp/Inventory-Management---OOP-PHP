		...
		<div class="wrapper">
			...
			<table class="tbl_one">
					...
					if($_SERVER['REQUEST_METHOD'] == 'POST'){
						...
						if(empty($name) or empty($date) or empty($description)){
							echo "<span style='color:#e53d37'>Error...Field Must Not Be Empty</span>";
						} else {
							$addspe = $user->addSpecial($name,$date,$description);
						    if($addspe){
								echo "<span style='color:green'>Information Added</span>";
							} 
						}
					}
				?>	
		           <tr>
					...
					<?php 
			   $getSpe = $user->getSpeById();
			   foreach($getSpe as $user){
			?>				   					
					<tr>
					  ...
					  <td><a href="profile.php?action=delete&id=<?php echo $user['id']; ?>">Delete</a></td>
					</tr>
			<?php } ?>		
               </table>
	           </br></br><div class="random_table"> 
<form action="" method="post">
					<table class="tbl_invoice">
						...
						<tr>
							<td>Name</td>
							<td><input type="text" name="name" placeholder="Please Enter Occasion Name" /></td>
						</tr>
