...
<html lang="en">
	...
	<body>
		<div class="wrapper">
				...
				<?php
					if($_SERVER['REQUEST_METHOD'] == 'POST'){
						...
						if(empty($name)){
							...
							$addcat = $user->addCat($name);
						    if($addcat){
								echo "<span style='color:green'>Category Added</span>";
							}  else {
								echo "<span style='color:#e53d7'>Category already exist</span>";
							}
						}
					}
				?>			 
			 </p>
			   </br><div class="random_table"> 
			<form action="" method="post">
					<table>
						<tr>
							<td>Name</td>
							<td><input type="text" name="catName" placeholder="Please Enter Category Name" /></td>
						</tr>
			...
			<?php
			$i=1;
			   $getCat = $user->getCatById();
			   foreach($getCat as $user){
			?>
				<tr>
					<td><?php echo $i; ?></td>
					<td><?php echo $user['catID']; ?></td>
					<td><?php echo $user['catName']; ?></td>
					<td><a href="edit_category.php?id=<?php echo $user['catID']; ?>">Edit</a></td>
					<td><a href="add_category.php?action=delete&id=<?php echo $user['catID']; ?>">Delete</a></td>
				</tr>	
			   <?php $i++;}  ?>
               </table>
			</div>
