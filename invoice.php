			...
			<div class="content">
			 ...
			 <p class="msg">  
				...
				if(isset($_POST['add'])){
				 ...
				 if(isset($_POST['add'])){
						if(empty($date) or empty($ssname) or empty($proid) or empty($quantity) or empty($uprice)){
							echo "<span style='color:#e53d37'>Error...Field Must Not Be Empty</span>";
				 } else {
							$addin = $user->addIN($date,$id,$ssname,$proid,$quantity,$uprice,$discount,$price);
						    if($addin){
                                   echo "<span style='color:green'>Invoice Added</span>";
							}
						}
				 }
					}
					...
					<td>
						<select id="select" name="id">
						   <option>Select Customer</option>
						             <?php
			                           $getCusIn = $user->getAllCus();
										foreach($getCusIn as $user){
			                            ?>
...
var el = document.getElementById('display');
 document.getElementById('showVal').onclick = function () {
        select = document.getElementById('proID');
			 ...
			 <a href="ch_in.php" target="_blank">**Check Invoice(by date)**</a>
			 </div>	
