	...
	<body>
		<div class="wrapper">
			...
			<div class="content">
			 ...
			 <table class="tbl_inv">
					  <form action="rep_gen.php" method="post">
					        <label> <h4>Report Generate of Threshold</label>
							<label> <h4>Minimum Threshold</label>
					  <input type="number" name="thMin" value="5" /><br>
					  <label> <h4>Maximum Threshold</label>
					  <input type="number" name="thMax" value="50" /><br>
				...
				<?php		
                 if($_SERVER['REQUEST_METHOD'] == 'POST'){
					 $thMin = $_POST['thMin'];
					 $thMax = $_POST['thMax'];
		if(isset($thMin) || isset($thMax)){	 
			   $getPro = $user->getProd();
			   foreach($getPro as $user){
				   @$b = $user['proID'];
				   while(@$b){
					   @$c = @$b+ 10;
				   }
